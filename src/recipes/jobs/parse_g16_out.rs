...
impl JobTemplate for ParseG16Out {
    ...
    fn instantiate(&self, ctx: &JobCtx<'_>) -> Result<JobArtifacts, RecipeError> {
        ...
        // v2 stage 2b: per-task common.toml for schema uniformity. parse_g16_out
        // runs no g16 / srun, so launcher / g16_cmd / scratch_root are empty and
        // module_profile is "default"; parse.py itself reads no site config.
        let common_toml = format!(
            "# Generated by `jm new`. Per-task job-manager site config (parse step).\n\
             # NOT the gaussian-compute-runtime --config schema (audit §6). The parse\n\
             # step runs no g16 / srun, so launcher / g16_cmd / scratch_root are empty.\n\
             launcher       = \"\"\n\
             g16_cmd        = \"\"\n\
             scratch_root   = \"\"\n\
             conda_env      = {}\n\
             ...
             pixi_manifest  = {}\n",
            toml::Value::String(pv(ctx, "conda_env").to_string()),
            toml::Value::String(pv(ctx, "pixi_manifest").to_string()),
        );
        ...
        // v2 stage 2b: conda_env / pixi_manifest are site config → common.toml.
        // parse has no experiment params, so plan.toml `[jobs.parse]` is empty.
        let plan_params = BTreeMap::new();
...
mod tests {
    ...
    fn instantiate_wires_input_and_emits_parse_py() {
        ...
        // stage 2b: a common.toml is emitted for schema uniformity (parse runs
        // no g16/srun, so the exec keys are empty); plan.toml is empty.
        let common = a
            .sidecars
            .iter()
            .find(|f| f.relpath.ends_with("parse/common.toml"))
            .unwrap();
        assert!(common.contents.contains("launcher       = \"\""));
        assert!(common.contents.contains("conda_env      = \"analysis\""));
        assert!(common.contents.contains("module_profile = \"default\""));
        assert!(
            a.plan_params.is_empty(),
            "stage 2b: parse has no experiment params; plan.toml [jobs.parse] is empty"
        );
    }
}
