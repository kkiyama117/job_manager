...
/// v2 stage 2b: site-config keys that move from `plan.toml` (JM_PARAM_*) into
/// the per-task `common.toml`. Excluded from `plan_params`.
const SITE_CONFIG_KEYS: &[&str] = &[
    "launcher",
    "g16_cmd",
    "scratch_root",
    "conda_env",
...
/// Render the per-task `common.toml` (job-manager site config). `run.py` reads
/// this via tomllib. NOT the gaussian-compute-runtime `--config` schema (audit
/// §6); the name collision is intentional and documented in v2 spec §4.3.
fn render_common_toml(ctx: &JobCtx<'_>) -> String {
    let q = |k: &str| toml::Value::String(pv(ctx, k).to_string()).to_string();
    format!(
        "# Generated by `jm new`. Per-task job-manager site config — read via\n\
         # tomllib by scripts/<id>.py. NOTE: this is NOT the gaussian-compute-\n\
         # runtime --config schema (audit §6); the name collision is benign.\n\
         # WARNING: no flow_dir-relative absolute paths — only cluster-fixed\n\
         # absolute paths (scratch_root, module bases) belong here.\n\
         launcher       = {}\n\
         g16_cmd        = {}\n\
         scratch_root   = {}\n\
         conda_env      = {}\n\
...
impl JobTemplate for G16Opt {
    ...
    fn instantiate(&self, ctx: &JobCtx<'_>) -> Result<JobArtifacts, RecipeError> {
        ...
        // v2 stage 2b: site config moves from plan.toml (JM_PARAM_*) into
        // <job_id>/common.toml, which run.py reads via tomllib. The bash
        // preamble still bakes conda_env / module_profile at scaffold time
        // (common.toml edits are not auto-synced into the bash).
        let common_toml = render_common_toml(ctx);
        ...
        let sidecars = vec![
            ...
            GeneratedFile {
                relpath: nsp("common.toml"),
                contents: common_toml,
                unix_mode: None,
            },
            GeneratedFile {
        ...
        // plan.toml keeps only experiment params; the 6 site-config keys live
        // in common.toml now (input_coordinate is scaffold-time only).
        let mut plan_params = BTreeMap::new();
        for rp in PARAMS {
            if rp.name == "input_coordinate" || SITE_CONFIG_KEYS.contains(&rp.name) {
                continue;
            }
...
mod tests {
    ...
    fn instantiate_emits_r4_body_and_sidecars() {
        ...
        // stage 2b: per-task common.toml carries the site config.
        let common = a
            .sidecars
            .iter()
            .find(|f| f.relpath.ends_with("opt/common.toml"))
            .unwrap();
        assert_eq!(common.unix_mode, None);
        assert!(common.contents.contains("launcher       = \"srun\""));
        assert!(common.contents.contains("g16_cmd        = \"g16\""));
        assert!(common.contents.contains("scratch_root   = \"\""));
        assert!(common.contents.contains("conda_env      = \"analysis\""));
        assert!(common.contents.contains("module_profile = \"gaussian_A\""));
        assert!(common.contents.contains("pixi_manifest  = \"\""));
        assert!(
            !common.contents.contains("/work/root/"),
    ...
    #[test]
    fn plan_params_keep_experiment_params_drop_site_config() {
        let params = default_params();
        let inputs = BTreeMap::new();
        let uuid = uuid::Uuid::now_v7();
        let a = G16Opt
            .instantiate(&ctx_with(&params, &inputs, &uuid))
            .unwrap();
        // Experiment params stay in plan.toml, typed.
        assert!(!a.plan_params.contains_key("input_coordinate"));
        assert_eq!(a.plan_params.get("charge"), Some(&toml::Value::Integer(0)));
        assert_eq!(a.plan_params.get("nproc"), Some(&toml::Value::Integer(8)));
        assert!(a.plan_params.contains_key("route"));
        assert!(a.plan_params.contains_key("compound"));
        // stage 2b: site config moved to common.toml — gone from plan.toml.
        for k in SITE_CONFIG_KEYS {
            assert!(
                !a.plan_params.contains_key(*k),
                "site-config key {k:?} must not appear in plan.toml"
            );
        }
    }
}
