...
ユーザ意向(2026-05-20):
- v2 では(c)`python -m gaussian_compute_runtime <step>` 起動への swap-in 経路を
...
5. **`# REPLACE_ME` swap-in 維持**(任意で `python -m gaussian_compute_runtime
   <step> --common "$JM_JOB_DIR/common.toml" --uuid "$JM_FLOW_UUID" --job-id
   "$JM_JOB_ID"` へ手動差替可能)。デフォルトは self-contained。
...
```toml
# Generated by `jm new <recipe>`. Per-task site config — values used by
# scripts/<id>.py (read via tomllib + os.environ) or by the optional
# `python -m gaussian_compute_runtime <step> --common ...` swap-in.
...
| gem stack | **任意 swap-in(`# REPLACE_ME` 維持)** | v2 デフォルト = `python -m gaussian_compute_runtime` | v1 self-contained 哲学維持、未導入サイトでも動く |
...
- `python -m gaussian_compute_runtime` を **v3 デフォルト**(gem stack 導入が広まれば)。
